...
def generate_launch_description():
    ...
    launch_yolo = TimerAction(
        ...
        actions=[
            IncludeLaunchDescription(
                ...
                launch_arguments={
                    'model': '/home/ntu-user/ros2_ws/src/NTU_COMP30271_CW_RobotSim/ntu_robotsim/models/custom_models/best_n.pt',
                    'device': 'cuda:0',
                    'threshold': '0.5',
    ...
    # Step J: YOLO Detection Logger
    logger_script = os.path.join(
        pkg_ntu_robotsim, 'launch', 'detection_logger.py')
    launch_logger = TimerAction(
        period=20.0,
        actions=[
            ExecuteProcess(
                cmd=[
                    'gnome-terminal', '--',
                    'bash', '-c',
                    f'python3 {logger_script}; exec bash'
                ],
                output='screen',
            )
        ]
